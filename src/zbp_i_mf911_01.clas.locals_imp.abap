CLASS lhc_header DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PUBLIC SECTION.
    CONSTANTS: gcf_status_normal TYPE c LENGTH 3 VALUE '正常',
               ...
               gcf_label_info    TYPE string VALUE 'ZC_MF911_02'.
...
CLASS lhc_header IMPLEMENTATION.
  ...
  METHOD getDataFile.
    DATA: lt_headers     TYPE TABLE FOR READ RESULT zi_mf911_01\\header,
          lt_items       TYPE TABLE FOR CREATE zi_mf911_01\\item,
          lt_del         TYPE TABLE FOR DELETE zi_mf911_01\\item,
          lt_existing    TYPE TABLE FOR READ RESULT zi_mf911_01\\item.
    ...
    LOOP AT lt_headers INTO DATA(ls_header).
      DATA lt_file_data TYPE zcl_mf911_01=>gtt_file_data.
      ...
      IF lt_file_data IS NOT INITIAL.
        ...
        LOOP AT lt_file_data INTO DATA(ls_line).
          DATA(ls_item) = zcl_mf911_01=>convert_data_file( ls_line ).
          APPEND VALUE #(
            %cid_ref = lv_cid_ref
            %target  = VALUE #( (
              %cid              = |ITEM_{ sy-tabix }|
  ...
  METHOD updateBusinessObject.
    DATA: lt_headers    TYPE TABLE FOR READ RESULT zi_mf911_01\\header,
          lt_upd_header TYPE TABLE FOR UPDATE zi_mf911_01\\header,
          lt_upd_item   TYPE TABLE FOR UPDATE zi_mf911_01\\item.
    ...
    LOOP AT lt_headers INTO DATA(ls_header).
      ...
      DATA lt_groups TYPE SORTED TABLE OF zi_mf911_02-group
      ...
      LOOP AT lt_groups INTO DATA(lv_group).
        DATA(lo_parallel) = NEW zcl_mf911_01( ).

        " Build item sub-table for this group
        DATA lt_group_items TYPE zcl_mf911_01=>gtt_item.
        LOOP AT lt_items INTO DATA(ls_grp_item) WHERE Group = lv_group.
          APPEND VALUE zcl_mf911_01=>gts_item_line(
        ...
        DATA(ls_input) = VALUE zcl_mf911_01=>gts_parallel_input(
          data = VALUE zcl_mf911_01=>gts_data(
            cid   = |GROUP_{ lv_group }|
            group = lv_group
        ...
        LOOP AT ls_result-data INTO DATA(ls_res).
          ...
          CASE ls_res-criticality
            WHEN zcl_mf911_01=>gcf_criticality_error.
              lv_error += 1.
            WHEN zcl_mf911_01=>gcf_criticality_warning.
              lv_warning += 1.
            WHEN OTHERS.
              lv_success += 1.
          ENDCASE.
